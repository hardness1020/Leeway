...
from leeway.tasks.types import TaskRecord
...
class AgentSpawner:
    ...
    async def spawn(self, spec: AgentSpec) -> TaskRecord:
        ...
        if spec.workflow:
            ...
            task = await self._task_manager.create_exec_task(
                args=[sys.executable, "-m", "leeway", "-p", spec.task, "--output-format", "text"],
                cwd=cwd,
                description=f"agent: {spec.task[:60]}",
            )
    ...
    async def _create_worktree(self, base_cwd: str) -> str:
        ...
        worktree_path = Path(tempfile.mkdtemp(prefix="leeway-wt-"))
