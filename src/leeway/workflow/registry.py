...
from leeway.workflow.types import WorkflowDefinition
...
def get_user_workflows_dir() -> Path:
    """Return ``~/.leeway/workflows/``, creating it if needed."""
...
def load_workflow_registry(cwd: str | Path | None = None) -> WorkflowRegistry:
    ...
    # User-level: ~/.leeway/workflows/*.yaml
    _scan_dir(get_user_workflows_dir(), registry)

    # Project-level: <cwd>/.leeway/workflows/*.yaml
    if cwd is not None:
        project_dir = Path(cwd).resolve() / ".leeway" / "workflows"
