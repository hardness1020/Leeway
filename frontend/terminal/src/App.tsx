...
const rawReturnSubmit = process.env.AGENTTREE_FRONTEND_RAW_RETURN === '1';
const scriptedSteps = (() => {
	const raw = process.env.AGENTTREE_FRONTEND_SCRIPT;
	if (!raw) {
		return [] as string[];
	}
	try {
...
export function App({config}: {config: FrontendConfig}): React.JSX.Element {
	...
	useInput((chunk, key) => {
		...
		// --- Escape while busy → cancel ---
		if (session.busy) {
			if (key.escape) {
				session.sendRequest({type: 'cancel'});
